...
def main():
    ...
    if enable_analyze:
        ...
        with st.spinner("Performing analysis..."):
            for sym, hold in st.session_state.portfolio.items():
                tech, forecast, news, counts, summaries = None, None, None, {}, {}
                try:
                    status_box.info(f"🔄 [{sym}] Calculating technical indicators...")
                    tech = get_technical_indicators(sym)
                    ...
                    positive_corpus = ' '.join(
                        [row[1]['content'][:1024] for row in news.iterrows() if row[1]['sentiment_label'] == 'positive']
                    )
                    negative_corpus = ' '.join(
                        [row[1]['content'][:1024] for row in news.iterrows() if row[1]['sentiment_label'] == 'negative']
                    )
                    neutral_corpus = ' '.join(
                        [row[1]['content'][:1024] for row in news.iterrows() if row[1]['sentiment_label'] == 'neutral']
                    )
                    ...
                    status_box.info(f"🤖 [{sym}] Generating final recommendation...")
                    rec_df = recommend(sym, tech, forecast,
                                       {'counts': counts, 'summaries': summaries, 'news': news},
                                       hold)
                except Exception as e:
                    status_box.error(f"❌ [{sym}] Analysis failed: {e}")
                    ...
                    rec_dict = rec_df.iloc[0].to_dict() if not rec_df.empty else {}
                    st.session_state.analysis[sym] = {
                        'tech': tech, 'forecast': forecast, 'news': news,
                        'counts': counts, 'summaries': summaries, 'recommendation': rec_dict
                    }
                    status_box.success(f"✅ [{sym}] Analysis complete.")
            status_box.success("🎉 All stock analyses completed successfully.")
        ...
        for sym, analysis in st.session_state.analysis.items():
            ...
            with st.container():
                ...
                with st.expander("📈 Recommendation", expanded=True):
                    ...
                    sentiment = 'positive' if reco in ["Buy", "Accumulate"] else 'negative' if "Sell" in reco else 'neutral'
                    st.markdown("##### News Summary:")
                    st.write(analysis['summaries'].get(sentiment, 'No summary available.'))

                    trend = rec.get('Forecast Trend')
                    text = '> 1.5%' if trend == 'Upward' else '< -1.5%' if trend == 'Downward' else '(-1.5% to +1.5%)'
                    color = 'green' if trend == 'Upward' else 'red' if trend == 'Downward' else 'blue'
                    st.markdown(f"##### <span style='color:{color}'>{trend} - {text}</span>", unsafe_allow_html=True)
    ...
    with tab2:
        ...
        with col1:
            try:
                ...
                pie = px.pie(names=list(alloc.keys()), values=list(alloc.values()), title="Portfolio Value Distribution")
                st.plotly_chart(pie, use_container_width=True)
            except KeyError as e:
                st.error(f"Missing data for allocation chart: {e}")
